...
class Graph:
    ...
    def add_edge(self, source: str, target: str) -> None:
        ...
        if item not in self.edges and (item[1], item[0]) not in self.edges and item[0] != item[1]:
    ...
    def max_degree_nodes(self) -> dict:
        ...
        # break edges into component nodes
        big_list = list()
        for x in self.edges:
            big_list.append(x[0])
            big_list.append(x[1])

        lst_1 = [(big_list.count(x), x) for x in set(big_list)] # get most common and max count
        ...
        for ele in lst_1:
            if ele[0] == max_count:
                result_dict.update({ele[1]:ele[0]})
...
class  TMDBAPIUtils:
    ...
    def get_movie_cast(self, movie_id:str, limit:int=None, exclude_ids:list=None) -> list:
        ...
        # print(f'Accessing cast API for movie: {movie_id}...')
        URL = 'https://api.themoviedb.org/3/movie/'+ movie_id + '/credits?api_key=0a79cd36709ce6616e4a5d49ea747923&language=en-US'
        ...
        result_list = list()
        for x in response_dict['cast']:
            if int(x['order']) < limit and int(x['id']) not in exclude_ids:
                entry_dict = {'id': str(x['id']), 'character': str(x['character']), 'credit_id': str(x['credit_id']), 'name': str(x['name'])}
    ...
    def get_movie_credits_for_person(self, person_id:str, vote_avg_threshold:float=None)->list:
        ...
        # print(f'Accessing credits API for actor: {person_id}...')
        URL = 'https://api.themoviedb.org/3/person/'+ person_id + '/movie_credits?api_key=0a79cd36709ce6616e4a5d49ea747923&language=en-US'
        httprequest = Request(URL, headers={"Accept": "application/json"})
        ...
        for x in response_dict['cast']:
            if 'vote_average' in x:
                if float(x['vote_average']) >= vote_avg_threshold:
                    entry_dict = {'id': str(x['id']), 'title': str(x['title']), 'vote_average': float(x['vote_average'])}
                    result_list.append(entry_dict)
...
def return_argo_lite_snapshot()->str:
    ...
    """
    return('https://poloclub.github.io/argo-graph-lite/#d6eda15d-f8bd-4a47-87cd-5ba35ecf04ca')
...
if __name__ == "__main__":
    ...
    tmdb_api_utils = TMDBAPIUtils(api_key='0a79cd36709ce6616e4a5d49ea747923')

    # call functions or place code here to build graph (graph building code not graded)
    # Suggestion: code should contain steps outlined above in BUILD CO-ACTOR NETWORK


    nodes_added_to_base = list()
    edges_added_to_base = list()

    credits = tmdb_api_utils.get_movie_credits_for_person('2975', 8)

    for x in credits:
        movie_id = x['id']
        cast = tmdb_api_utils.get_movie_cast(movie_id, limit = 3)
        for y in cast:
            cast_id = y['id']
            graph.add_node(cast_id, y['name'])
            nodes_added_to_base.append(cast_id)
            graph.add_edge('2975', cast_id)
            edges_added_to_base.append(('2975', cast_id))

    loops = 0
    new_nodes = list()
    while loops <= 1: # change this to 1 **********************
        if loops == 0:
            nodes = nodes_added_to_base
        else:
            nodes = new_nodes
            new_nodes = list()

        for x in nodes:
            credits = tmdb_api_utils.get_movie_credits_for_person(x, vote_avg_threshold=8)

            for y in credits:
                movie = y['id']
                cast = tmdb_api_utils.get_movie_cast(movie, limit=3)
                ...
                for z in cast:
                    ...
                    if name not in graph.nodes:
                        graph.add_node(id = cast_id, name = name)
                        new_nodes.append(cast_id)

                    if (name, x) not in graph.edges and (x, name) not in graph.edges and x != name:
                        graph.add_edge(x, cast_id)
